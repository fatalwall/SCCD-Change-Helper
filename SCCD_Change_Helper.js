...
function CreateCalendarButton() {	
	if (document.getElementById("mxb2c5b3bc").value != "DRAFT"
		...
		SendToCalendarButton.setAttribute("src", chrome.extension.getURL("Images/calendar.png"));
...
function CreateEmailButton() {
	...
	EmailOwnerButton.setAttribute("src", chrome.extension.getURL("Images/email.png"));
...
{
	if (getVariableValue('uniqueid') != undefined || getFieldValue('mx9d44d398') != undefined) {
		return true;
	} else { 
		return false; 
	}	
}
function isChangeRecordPage()
{
	if (getVariableValue('value')=='change') {
		return true; 
	} else { 
		return false; 
	}	
...
//Only add Buttons if a record is being displayed
if (isRecordPage() == true){
	//Only add Calendar buttons if its a Change Record
	if (isChangeRecordPage() == true) {
		CreateEmailButton();
